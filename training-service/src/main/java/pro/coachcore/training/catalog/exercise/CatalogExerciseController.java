...
public class CatalogExerciseController {
  ...
  public ResponseEntity<List<CatalogExerciseDto>> getCatalogExercises() {
    var exercises = catalogExerciseService.findAllCatalogExercises();
    var exercisesDto = exercises.stream().map(catalogExerciseMapper::map).toList();

    return ResponseEntity.ok(exercisesDto);
  }

  @PostMapping
  public ResponseEntity<CatalogExerciseDto> postCatalogExercise(@RequestParam String categoryId,
      @RequestBody CreateCatalogExerciseDto dto) throws ResourceNotFoundException {
